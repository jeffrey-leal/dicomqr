...
func encodeQRPayload(opType qrOpType, qrLevel QRLevel, filter []*dicom.Element, cm *contextManager) (contextManagerEntry, []byte, error) {
	...
	case QRLevelPatient:
		...
		// Patient Root SOP class is used for the association context, but
		// QueryRetrieveLevel is STUDY — Patient Root supports study-level
		// queries and this app always retrieves studies, not patients.
		qrLevelString = "STUDY"
	...
	if err != nil {
		// PACS rejected the primary SOP class — try the other info model.
		var fallbackUID string
		switch sopClassUID {
		case dicomuid.StudyRootQRFind:
			fallbackUID = dicomuid.PatientRootQRFind
		case dicomuid.StudyRootQRMove:
			fallbackUID = dicomuid.PatientRootQRMove
		case dicomuid.StudyRootQRGet:
			fallbackUID = dicomuid.PatientRootQRGet
		case dicomuid.PatientRootQRFind:
			if qrLevelString != "PATIENT" {
				fallbackUID = dicomuid.StudyRootQRFind
			}
		case dicomuid.PatientRootQRMove:
			if qrLevelString != "PATIENT" {
				fallbackUID = dicomuid.StudyRootQRMove
			}
		case dicomuid.PatientRootQRGet:
			if qrLevelString != "PATIENT" {
				fallbackUID = dicomuid.StudyRootQRGet
			}
		}
		if fallbackUID != "" {
			context, err = cm.lookupByAbstractSyntaxUID(fallbackUID)
		}
		if err != nil {
	...
	// Collect all identifier elements, appending QueryRetrieveLevel if absent.
	allElems := make([]*dicom.Element, 0, len(filter)+1)
	foundQRLevel := false
	...
	sort.Slice(allElems, func(i, j int) bool {
		ti, tj := allElems[i].Tag, allElems[j].Tag
		if ti.Group != tj.Group {
			return ti.Group < tj.Group
		}
		return ti.Element < tj.Element
	})
...
func (su *ServiceUser) CFind(qrLevel QRLevel, filter []*dicom.Element) chan CFindResult {
	...
	go func() {
		...
		for {
			...
			// response (0x0000) is a terminator with no dataset and must not
			// be emitted as a result.
			isPending := resp.Status.Status == dimse.StatusPending ||
				resp.Status.Status == 0xFF01
			if isPending {
			...
			if !isPending {
				if resp.Status.Status != dimse.StatusSuccess {
					ch <- CFindResult{Err: fmt.Errorf("C-FIND failed with status %04XH: %v", uint16(resp.Status.Status), resp.Status.ErrorComment)}
				}
				break
			}
		}
...
// CMoveProgress holds sub-operation counts delivered in each C-MOVE-RSP pending
// response. All counts are zero until the PACS begins dispatching sub-operations.
...
// CMove issues a C-MOVE-RQ, directing the PACS to push matching files to
// moveDestination. onProgress is invoked for every C-MOVE-RSP pending response
// and may be nil. Returns nil when the final response carries StatusSuccess.
//
// REQUIRES: Connect() or SetConn has been called.
func (su *ServiceUser) CMove(qrLevel QRLevel, moveDestination string, filter []*dicom.Element, onProgress func(CMoveProgress)) error {
	err := su.waitUntilReady()
	if err != nil {
		return err
	}
	...
	cs.sendMessage(
		&dimse.CMoveRq{
			AffectedSOPClassUID: context.abstractSyntaxUID,
			MessageID:           cs.messageID,
			Priority:            0x0000, // MEDIUM per PS3.7 C.3.2.1
	...
	for {
		...
		if !ok {
			su.status = serviceUserClosed
			return fmt.Errorf("dicom.serviceUser: connection closed waiting for C-MOVE response")
		}
		resp, ok := event.command.(*dimse.CMoveRsp)
		if !ok {
			return fmt.Errorf("dicom.serviceUser: unexpected response for C-MOVE: %v", event.command)
		}
		if onProgress != nil {
